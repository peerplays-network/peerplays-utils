...
```
If a image with particular tag is to be pulled, then goto Packages&Registries --> Container Registry, where the images available are displayed. Click on the image needed and a page where all the available tags(versions) for the images are shown. Use the tag or click on the clipboard symbol on the side of the image name. Then run the command
```
